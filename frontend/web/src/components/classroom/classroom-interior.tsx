...
interface ParticipantDisplay {
  id: string; name: string; role: string; isTeacher?: boolean; isLocal?: boolean;
}
...
export function ClassroomInterior({ roomId, onClose }: ClassroomInteriorProps) {
  ...
  const allParticipants: ParticipantDisplay[] = [
    ...(user ? [{ id: 'local', name: userName, role: user.role, isTeacher: user.role === 'teacher', isLocal: true }] : []),
    ...remoteParticipants.map(p => ({ id: p.id, name: p.name || p.userId, role: 'student' as const })),
  ]
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-white border-b border-gold/20">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-gold">{isConnected ? '🟢 Live' : '⏳ Connecting...'} · {formatTime(seconds)}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setSidePanel(sidePanel === 'chat' ? null : 'chat')}
            className="p-2 hover:bg-gray-100 rounded-full text-gray-600">💬</button>
          <button onClick={() => setSidePanel(sidePanel === 'participants' ? null : 'participants')}
            className="p-2 hover:bg-gray-100 rounded-full text-gray-600">👥</button>
          <button onClick={onClose}
            className="p-2 hover:bg-red-50 rounded-full text-red-500">✕</button>
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="flex-1 grid grid-cols-5 grid-rows-4 gap-1 p-2 bg-gray-50/80">
          {allParticipants.slice(0, 4).map((p, i) => (
            <div key={p.id}
              className={`rounded-xl shadow-sm border flex flex-col items-center justify-center relative overflow-hidden
                ${p.isTeacher ? 'border-2 border-gold shadow-gold/20' : 'border-gray-100'}
                ${p.isLocal ? 'ring-2 ring-gold/40' : ''}`}
              style={{ gridRow: 1, gridColumn: i + 1 }}>
              {p.isLocal && localStream ? (
                <video ref={localVideoRef} autoPlay playsInline muted
                  className="absolute inset-0 w-full h-full object-cover" />
              ) : null}
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-gold to-gold-dark flex items-center justify-center text-white text-xs font-bold z-10">
                {p.name[0]}
              </div>
              <span className="text-[9px] font-medium text-navy mt-0.5 z-10">{p.name}</span>
            </div>
          ))}
          ...
          {allParticipants.slice(4, 8).map((p, i) => (
            <div key={p.id}
              className="rounded-xl bg-white shadow-sm border border-gray-100 flex flex-col items-center justify-center relative"
              ...
              {sidePanel === 'chat' ? (
                ...
                allParticipants.map(p => (
                  <div key={p.id} className="flex items-center gap-2 text-sm p-1.5 hover:bg-gray-50 rounded-lg">
                    ...
                    <span className="text-xs">{p.name} {p.isTeacher ? '👑' : ''} {p.isLocal ? '(you)' : ''}</span>
                  </div>
                ))
              )}
            </div>
