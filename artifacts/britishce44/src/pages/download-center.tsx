...
const PLATFORMS = [
  ...
  { os: 'macOS', icon: '🍎', size: '72 MB', ver: '2.1.0', color: '#c8940a', note: 'Apple Silicon & Intel' },
  ...
  { os: 'Linux', icon: '🐧', size: '58 MB', ver: '2.1.0', color: '#a78bfa', note: 'AppImage / .deb' },
]
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl p-5 relative overflow-hidden"
        style={{ background: 'linear-gradient(120deg,rgba(27,62,166,0.4),rgba(13,20,37,0.7))', border: '1px solid rgba(37,99,235,0.3)' }}>
        <div className="absolute -top-12 -right-12 w-48 h-48 rounded-full opacity-20" style={{ background: 'radial-gradient(circle,#c8940a,transparent 70%)' }} />
        <div className="relative flex items-center justify-between flex-wrap gap-4">
          ...
          <button onClick={() => setInstalled(true)}
            ...
            style={{ background: installed ? 'rgba(52,211,153,0.2)' : 'linear-gradient(90deg,#c8940a,#e0a800)', border: installed ? '1px solid rgba(52,211,153,0.4)' : 'none' }}>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {PLATFORMS.map((p, i) => (
            <motion.div key={p.os} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              className="rounded-2xl p-4 relative overflow-hidden" style={{ background: 'rgba(13,20,37,0.7)', border: `1px solid ${p.color}22` }}>
